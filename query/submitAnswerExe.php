...
$selExAttempt = $conn->query("SELECT * FROM exam_attempts WHERE student_id='$student_id' AND exam_id='$exam_id'  ");
...
else if($selAns->rowCount() > 0)
{
	...
	if($updLastAns)
	{
		...
		if($insAns)
		{
			 $insAttempt = $conn->query("INSERT INTO exam_attempts(student_id,exam_id)  VALUES('$student_id','$exam_id') ");
			 if($insAttempt)
			 {
